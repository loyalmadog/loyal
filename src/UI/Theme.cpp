#include "Theme.h"
#include "imgui_internal.h" // Nécessaire pour un meilleur contrôle du dessin
...
namespace UI {
    void ApplyModernTheme() {
        ...
        // Configuration pour ressembler au dashboard OrbDiff
        style.WindowRounding    = 0.0f;
        style.ChildRounding     = 4.0f;
        style.FrameRounding     = 4.0f;
        style.PopupRounding     = 4.0f;
        
        style.WindowPadding     = ImVec2(0.0f, 0.0f); // Le padding géré manuellement
    ...
    bool DrawNavTab(const char* label, bool active) {
        ...
        // Ajouter de l'espacement autour du texte
        ...
        // Barre de soulignement jaune si actif
        if (active) {
    ...
    bool DrawToolCard(const char* title, const char* desc, float width, bool active) {
        ...
        ImVec2 pos = window->DC.CursorPos;
        ImVec2 size = ImVec2(width, 120.0f); // Hauteur fixe pour les cartes
